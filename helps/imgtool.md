...
所有图片操作均支持英文名和中文名，可以使用 `/img 操作名`、`/imgtool 操作名`
或直接使用 `/操作名`（详见[图片处理](#图片处理)）。
推荐在脚本、教程和可能存在命令冲突的场景中使用稳定的 `/img` 写法。
...
- [抽帧(extract)](#抽帧extract)
- [幻影坦克(mirage)](#幻影坦克mirage)
...
### 服务开关
🛠️ `/imgtool on` `/imgtool off`
`/imgtool status`
> 群管理员以上可以开启或关闭本群的图片处理服务；所有群成员都可以查询当前状态
...
操作之间只要前一步结果符合下一步需要的图片类型，就可以连续执行。
可用的操作见图片操作目录  
处理多张图片的方式：  
...
- `(回复一张图片) /img 旋转 90` 使用中文操作名逆时针旋转90度
- `(回复一张图片) /img rotate 90 resize 0.5x` 逆时针旋转90度，再缩小到50%
- `(回复一张图片) /img 旋转 90 缩放 0.5x` 同一条操作链可以混用中文名和英文名
...
### 快捷处理指令
`/英文操作名` `/中文操作名`
> 所有图片操作的英文名和中文名都可以省略 `img` 前缀；首个快捷指令会自动成为操作链的第一个操作，后面仍可继续串联其他操作。
> 如果快捷指令与其他插件冲突，请改用稳定的 `/img 操作名` 写法。

- `(回复一张图片) /rotate 90` 等价于 `/img rotate 90`
- `(回复一张图片) /旋转 90` 等价于 `/img 旋转 90`
- `(回复一张图片) /rotate 90 resize 0.5x` 等价于 `/img rotate 90 resize 0.5x`
- `(回复一张图片) /旋转 90 缩放 0.5x` 等价于 `/img 旋转 90 缩放 0.5x`
- `(回复多张图片) /concat h` 等价于 `/img concat h`
- `/gif` 会智能判断输入：回复静态图片时执行 `/img gif`，回复视频时执行视频转 GIF
- `/转gif` 是图片操作快捷指令，只处理静态图片；视频转 GIF 请继续使用 `/gif`
- 图片列表、帮助、检查和服务开关仍使用 `/img push`、`/img help`、`/img check`、`/imgtool on` 等原指令

英文操作名与中文操作名的对应关系如下：

| 英文操作名 | 中文操作名 | 作用 |
|---|---|---|
| `gif` | `转gif` | 将静态图片转换为 GIF |
| `png` | `转png` | 将图片转换为 PNG |
| `resize` | `缩放` | 按尺寸或比例缩放图片 |
| `mirror` | `镜像` | 水平或垂直镜像翻转 |
| `rotate` | `旋转` | 按指定角度旋转 |
| `back` | `倒放` | 反转动图时间轴 |
| `speed` | `倍速` | 调整动图速度或帧时长 |
...
| `fan` | `转动` | 生成持续旋转的动图 |
| `flow` | `流动` | 生成平移循环动图 |
| `concat` | `拼接` | 将多张图片拼接成静态图 |
| `stack` | `合成动图` | 将多张图片合成为动图 |
| `extract` | `抽帧` | 从动图中提取帧 |
| `mirage` | `幻影坦克` | 生成幻影坦克图片 |
| `demirage` | `还原幻影坦克` | 提取幻影坦克的表图和隐藏图 |
| `brighten` | `亮度` | 调整图片亮度 |
| `contrast` | `对比度` | 调整图片对比度 |
| `sharpen` | `锐度` | 调整图片锐度 |
| `saturate` | `饱和度` | 调整图片饱和度 |
| `blur` | `模糊` | 应用高斯模糊 |
| `crop` | `裁剪` | 按尺寸、比例和位置裁剪 |
| `cutout` | `抠图` | 移除纯色背景 |
| `shrink` | `裁剪透明` | 裁掉图片边缘的透明区域 |
| `bg` | `添加背景` | 为透明图片添加背景色 |


### 查看图片处理操作帮助
`/img help`
> 使用英文名或中文名查看某个图片操作的帮助信息

- `/img help rotate` 查看rotate操作的帮助信息
- `/img help 旋转` 查看同一操作的中文帮助入口
...
图片列表按“当前群聊 + 你的 QQ”分别保存，私聊列表也独立保存；超过 1 小时未使用会自动清空。
...
### Markdown转图片
`/md` `/markdown`
> 将 Markdown 文本转换为图片；出于安全考虑，不会加载文中的外部图片链接
...
### 视频转GIF
`/gif`
> 智能 GIF 快捷指令：回复静态图片时转换图片格式，回复视频时将视频转换为 GIF。
> 视频转换会按照指定上限缩小分辨率、降低帧率和限制帧数；同时包含图片和视频时会要求明确选择。

- `(回复一张静态图片) /gif` 等价于 `/img gif`
- `(回复一条视频消息) /gif`
- `(回复一条视频消息) /gif -s 512 -f 5 -n 100` 限制长边 512、最高 5 FPS、最多 100 帧
- 长边支持 64-1024，帧率支持 1-30，帧数支持 1-300；输入和输出文件均受 20MB 限制
...
- `(回复一张静态图片) /gif` 是 `/img gif` 的快捷写法；回复视频时 `/gif` 自动切换为视频转换
...
### 倍速(speed)
`动图` -> `动图`
> 调整动图速度并保留原图的逐帧停留时间；速度过快时会按播放时间抽帧，总时长不会被重复缩短
...
ai: AI抠图当前未启用，请使用默认的非AI抠图
洪水算法抠图时可以指定容差，默认为20  

- `cutout` 使用洪水算法抠图，容差为20
- `cutout 50` 使用洪水算法抠图，容差为50
- `cutout ai` 当前服务器未启用，会提示改用 `cutout`
