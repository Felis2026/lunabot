...
- 🔒 [开启/关闭消息统计定时发送](#开启关闭消息统计定时发送)
- 🔒 [取消当天消息统计定时发送](#取消当天消息统计定时发送)
- 🔒 [添加词汇到用户词典](#添加词汇到用户词典)
- 🔒 [添加词汇到停用词典](#添加词汇到停用词典)
...
🔒 `/sta on` `/sta off`
`/sta status`
> 仅 SUPERUSER 可以开启或关闭当前群的群消息统计服务；所有群成员都可以查询当前状态
...
### 开启/关闭消息统计定时发送
🔒 `/sta notify on` `/sta notify off`
`/sta notify status`
> 开启或关闭群消息统计通知，开启后每天会定时发送群消息统计图；所有群成员都可以查询当前状态
