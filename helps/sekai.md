...
`default` 自查时优先使用已授权的 Haruki OAuth 数据，失败时再回退 Public-API  
`oauth` 自查时只使用已授权的 Haruki OAuth 数据  
`public` 统一只使用 Public-API  
...
授权成功后，Bot 可在你自查时通过 Haruki OAuth 读取你的 Suite 数据；JP 服务器的 **MySekai** 相关查询也会使用该授权链路  
