...
# ================================ CR任务全量档位兼容层 ================================ #
# /cr任务 的 overview 画面仍在 profile.py 内，但 all 视图的参数解析和详览绘图
# 原本应由 education.py 提供。
# 当前仓库在整合时只接入了 profile 侧调用，遗漏了这里的实现，导致运行时 ImportError。
...
def extract_character_rank_all_flag(args: str) -> tuple[bool, str]:
    normalized_keywords = {_normalize_cr_mission_query(keyword) for keyword in CHARACTER_RANK_ALL_KEYWORDS}
    tokens = args.split()
    for i, token in enumerate(tokens):
        if _normalize_cr_mission_query(token) in normalized_keywords:
            return True, " ".join(tokens[:i] + tokens[i + 1:]).strip()

    normalized = _normalize_cr_mission_query(args)
    for keyword in CHARACTER_RANK_ALL_KEYWORDS:
        if _normalize_cr_mission_query(keyword) in normalized:
